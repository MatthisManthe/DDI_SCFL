...
def main(args, config):
    ...
    for restart in range(config["nb_restart"]):
        ...
        global_c = [torch.zeros_like(param.data) for param in model.parameters()]
        ...
        for comm in range(config["max_rounds"]):  # loop over the dataset multiple times
            ...
            for idc, c in enumerate(clients):
                ...
                for id_group, (client_c_group, global_c_group, local_model_group, global_model_group) in enumerate(zip(client_c[c], global_c, local_models[c].parameters(), model.parameters())):
                    delta_c[c][id_group] = -global_c_group + 1.0/num_batches/config["learning_rate"] * (global_model_group - local_model_group)
                    client_c[c][id_group] += delta_c[c][id_group]
            ...
            for c in clients:
                for id_group, delta_control_group in enumerate(delta_c[c]):
                    global_c[id_group] += delta_control_group * len(local_datasets[c])/len(train_data)

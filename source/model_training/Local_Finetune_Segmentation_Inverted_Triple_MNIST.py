...
def main(args, config):
    ...
    if config["generate_partitioning"] in ["train", "val", "test"]:
        if config["generate_partitioning"] == "train":
            gen_data_size = len(train_images)//2
        elif config["generate_partitioning"] == "val":
            gen_data_size = len(val_images)//2
        elif config["generate_partitioning"] == "test":
            gen_data_size = len(test_images)//2
        
        if config["iid"]:
            if not os.path.exists(os.path.join(config["dataset"], "IID_partitioning")):
                os.mkdir(os.path.join(config["dataset"], "IID_partitioning"))
            local_samples = Inverted_Triple_MNIST_iid(gen_data_size, clients, save_folder=os.path.join(config["dataset"], "IID_partitioning"), seed=0, split=config["generate_partitioning"])
            ...
            if config["fraction"] is not None:
                ...
                local_samples = Inverted_Triple_MNIST_Fraction_non_iid(gen_data_size, clients, config["fraction"], save_folder=os.path.join(config["dataset"], f"Fraction_non_IID_partitioning_{config['fraction']}"), seed=1)
                ...
                local_samples = Inverted_Triple_MNIST_Full_non_iid(gen_data_size, clients, save_folder=os.path.join(config["dataset"], "Full_non_IID_partitioning"), seed=1, split=config["generate_partitioning"])
                ...
                local_samples = Inverted_Triple_MNIST_Dirichlet_non_iid(gen_data_size, clients, config["dirichlet"], save_folder=os.path.join(config["dataset"], "Dirichlet_non_IID_partitioning"), seed=20, split=config["generate_partitioning"])
    else:
        ...
        with open(os.path.join(config["partitioning_dir"], config["test_partition_file"]), 'r') as f:
            test_local_samples = json.load(f)
        with open(os.path.join(config["iid_test_partition_file"]), 'r') as f:
            other_test_local_samples = json.load(f)
    ...
    assert_Inverted_Triple_MNIST_partitioning(val_local_samples, clients)
    assert_Inverted_Triple_MNIST_partitioning(test_local_samples, clients)
    assert_Inverted_Triple_MNIST_partitioning(other_test_local_samples, clients)
    
    train_dataset_size = len(train_images)//2
    local_datasets = {} 
    for c in clients:
        full_local_samples = []
        ...
        if "Inverted" in  local_samples[c]:
            full_local_samples.extend([ids+train_dataset_size for ids in local_samples[c]["Inverted"]])
    ...
    val_dataset_size = len(val_images)//2
    val_local_datasets = {} 
    for c in clients:
        full_local_samples = []
        if "Clear" in val_local_samples[c]:
            full_local_samples.extend(val_local_samples[c]["Clear"])
        if "Inverted" in  val_local_samples[c]:
            full_local_samples.extend([ids+val_dataset_size for ids in val_local_samples[c]["Inverted"]])
        val_local_datasets[c] = torch.utils.data.Subset(val_data, full_local_samples)
        
    val_local_loaders = {c:torch.utils.data.DataLoader(val_local_datasets[c], batch_size=config["batch_size"], shuffle=True, num_workers=0) for c in clients}
    
    test_dataset_size = len(test_images)//2
    test_local_datasets = {} 
    for c in clients:
        full_local_samples = []
        if "Clear" in test_local_samples[c]:
            full_local_samples.extend(test_local_samples[c]["Clear"])
        if "Inverted" in  test_local_samples[c]:
            full_local_samples.extend([ids+test_dataset_size for ids in test_local_samples[c]["Inverted"]])
        test_local_datasets[c] = torch.utils.data.Subset(test_data, full_local_samples)
        
    test_local_loaders = {c:torch.utils.data.DataLoader(test_local_datasets[c], batch_size=config["batch_size"], shuffle=True, num_workers=0) for c in clients}
    
    other_test_local_datasets = {} 
    for c in clients:
        full_local_samples = []
        if "Clear" in other_test_local_samples[c]:
            full_local_samples.extend(other_test_local_samples[c]["Clear"])
        if "Inverted" in  other_test_local_samples[c]:
            full_local_samples.extend([ids+test_dataset_size for ids in other_test_local_samples[c]["Inverted"]])
        other_test_local_datasets[c] = torch.utils.data.Subset(test_data, full_local_samples)
        
    other_test_local_loaders = {c:torch.utils.data.DataLoader(other_test_local_datasets[c], batch_size=config["batch_size"], shuffle=True, num_workers=0) for c in clients}
    ...
    for restart in range(config["nb_restart"]):
        ...
        for idc, c in enumerate(clients):
            ...
            local_models[c].load_state_dict(torch.load(os.path.join(config["previous_train_dir"], config["model_init"].replace("#", str(restart)))))
            ...
            with torch.no_grad():
                ...
                for idl, (images, labels) in enumerate(tqdm(val_local_loaders[c])):
                    images = images.to(device)
                    labels = labels.to(device)
                    
                    test_output = local_models[c](images)
                ...
                for cl in classes:
                    writer.add_scalar(f"Validation/Restart_{restart}_IoU_{cl}", report["IoUs"][cl], config["split_round"])
                writer.add_scalar(f"Validation/Restart_{restart}_mIoU", report["mIoU"], config["split_round"])
                
                if report["mIoU"] >= best_metric:
                    best_metric = report["mIoU"]
                    print("Saving new best model")
                    torch.save(local_models[c].state_dict(), os.path.join(log_dir, f"model_client_{c}_restart_{restart}.pth"))
                    
                    with open(os.path.join(log_dir, f'val_perf_client_{c}_restart_{restart}.json'), 'w') as f:
            ...
            for comm in range(config["split_round"], config["max_rounds"]):  # loop over the dataset multiple times
                ...
                with torch.no_grad():
                    ...
                    if comm+1 in config["save_rounds"]:
                        shutil.copy(os.path.join(log_dir, f"model_client_{c}_restart_{restart}.pth"), os.path.join(log_dir, f"model_client_{c}_restart_{restart}_round_{comm+1}.pth"))

        for c in clients:
            
            # Test the model
            model.load_state_dict(torch.load(os.path.join(log_dir, f"model_client_{c}_restart_{restart}.pth")))
            model.eval()
            
            with torch.no_grad():
    
                for idl, (images, labels) in enumerate(tqdm(test_local_loaders[c])):
                    images = images.to(device)
                    labels = labels.to(device)
        ...
        for c in clients:
            ...
            with torch.no_grad():
                ...
                for idl, (images, labels) in enumerate(tqdm(other_test_local_loaders[c])):
                    images = images.to(device)
                    labels = labels.to(device)
        ...
        iid_test_perfs.append(report["mIoU"])
                
    df_metric = pd.DataFrame(data={"test_mIoU":test_perfs, "iid_test_mIoU":iid_test_perfs})

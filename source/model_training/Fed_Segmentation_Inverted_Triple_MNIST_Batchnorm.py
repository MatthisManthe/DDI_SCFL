...
def main(args, config):
    ...
    for restart in range(config["nb_restart"]):
        ...
        model = FCN_Triple_MNIST_Segmentation(nb_classes=nb_classes, norm="batch").to(device)
        
        local_models = {c:FCN_Triple_MNIST_Segmentation(nb_classes=nb_classes, norm="batch").to(device) for c in clients}
        ...
        for comm in range(config["max_rounds"]):  # loop over the dataset multiple times
            ...
            for key in model.state_dict().keys():
                ...
                if 'num_batches_tracked' in key:
                    model.state_dict()[key].data.copy_(local_models["0"].state_dict()[key])
                else:
                    # Average parameters (and buffers ...)
                    temp = torch.zeros_like(model.state_dict()[key])
                    for c in clients:
                        temp += local_models[c].state_dict()[key] * len(local_datasets[c])/len(train_data)
                    model.state_dict()[key].data.copy_(temp)
                    # Communicate new state to each client
                    for c in clients:
                        local_models[c].state_dict()[key].data.copy_(model.state_dict()[key])
